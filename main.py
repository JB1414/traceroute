...
def get_arguments():
    parser = argparse.ArgumentParser(
        description="Traces the route to the server")
    parser.add_argument(
        "address", help="The destination adress to traceroute to", type=str)
    parser.add_argument(
        "-l", "--length", help='Set custom length of the packet', type=int, default=40)
    parser.add_argument("-seq", help="Custom SEQ", type=int, default=0)
    parser.add_argument("-t", "--timeout",
                        help="Responce Timeout", type=int, default=1)
    parser.add_argument("-i", "--interval",
                        help="Interval between requests", type=int, default=0)
    parser.add_argument(
        "-r", "--repeat", help="How many times repeat packets to each adress", type=int, default=3)
    parser.add_argument("-mh", "--maxhops",
                        help="Max hops count", type=int, default=30)
    parser.add_argument("-dm", "--debugmode",
                        help="Activates debug mode", action="store_true")
...
def main():
    ...
    tracert = Traceroute(packet, args.timeout, args.interval,
                         args.repeat, args.maxhops, args.debugmode)
    tracert.traceroute()
