...
ResponceInfo = namedtuple(
    "ResponceInfo", ["reciever_address", "responces_times_miliseconds"])
...
class Ping:
    ...
    def do_ping(self) -> ResponceInfo:
        ...
        for i in range(self.repeat):
            responce = sr1(
                self.packet, timeout=self.timeout_seconds, verbose=0)
            if responce is not None:
                responce_time_miliseconds = Ping.to_miliseconds(
                    responce.time - self.packet.sent_time)
                responces_times_miliseconds.append(
                    round(responce_time_miliseconds))
